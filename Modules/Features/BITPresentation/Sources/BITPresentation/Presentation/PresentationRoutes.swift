...
extension PresentationRoutes where Self: RouterProtocol {
  ...
  public func presentationResultState(with state: PresentationRequestResultState, context: PresentationRequestContext) {
    let module = PresentationRequestResultStateModule(state: state, context: context)
    let viewController = module.viewController
